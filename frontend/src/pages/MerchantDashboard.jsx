...
const MerchantDashboard = () => {
  ...
  const [analytics, setAnalytics] = useState({ totalSales: 0, categories: [] });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setAnalytics(aRes.data || { totalSales: 0, categories: [] });
      } catch (err) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <section className="p-4 border rounded-lg bg-white shadow-sm">
        ...
        <p className="text-sm text-slate-600">Total Sales: ₹{analytics.totalSales.toFixed(2)}</p>
        <div className="mt-3 space-y-1">
          {analytics.categories.map((c) => (
            <div key={c.category} className="flex justify-between text-sm">
              <span>{c.category}</span>
              <span>
                ₹{c.totalSales.toFixed(2)} ({c.receipts} receipts)
              </span>
            </div>
          ))}
